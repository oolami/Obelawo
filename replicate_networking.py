...
def assume_role_via_session(base_session: boto3.Session, role_arn: str, session_name: str) -> boto3.Session:
    """
    Use an existing session (DR) to assume a role in another account (Prod).
    No separate Prod profile needed.
    """
...
def translate_actions(actions: List[dict], tg_arn_map: Dict[str, str]) -> List[dict]:
    ...
    for act in actions:
        new_act = act.copy()
        if act["Type"] == "forward" and "TargetGroupArn" in act:
            prod_arn = act["TargetGroupArn"]
            if prod_arn in tg_arn_map:
                new_act["TargetGroupArn"] = tg_arn_map[prod_arn]
            else:
...
def main():
    ...
    prod_region  = cfg["prod_region"]
    dr_region    = cfg["dr_region"]
    prod_role_arn = cfg["prod_role_arn"]
    vpc_map      = cfg["vpc_map"]
    ...
    # DR base session (only profile you need to configure)
    print(f"Using AWS profile for DR (CREATE/MODIFY + AssumeRole into Prod): {dr_profile}")
    dr_base_sess = boto3.Session(profile_name=dr_profile)

    # Assume role into Prod using DR session (no Prod profile needed)
    print(f"Assuming role into Prod: {prod_role_arn}")
    prod_sess = assume_role_via_session(dr_base_sess, prod_role_arn, "ProdReadOnly")

    # STS sanity check
    prod_sts = prod_sess.client("sts")
    dr_sts   = dr_base_sess.client("sts")

    prod_acct = prod_sts.get_caller_identity()["Account"]
    dr_acct   = dr_sts.get_caller_identity()["Account"]

    print(f"Prod account (assumed): {prod_acct} (region {prod_region})")
    print(f"DR   account (profile): {dr_acct} (region {dr_region})")

    if prod_acct == dr_acct:
        raise RuntimeError("Prod and DR sessions are the SAME account. Aborting.")

    # Service clients
    prod_ec2 = prod_sess.client("ec2", region_name=prod_region)
    prod_elb = prod_sess.client("elbv2", region_name=prod_region)

    dr_ec2   = dr_base_sess.client("ec2", region_name=dr_region)
    dr_elb   = dr_base_sess.client("elbv2", region_name=dr_region)
